...
async function setupcontenidos() {
    ...
    wordSelection
        ...
        function handleAuthorClick(authorName) {
            ...
            fetch(`/authors-titles?tipo=${selectedType}&genero=${selectedGenre}&direccion=${selectedStage}`)
                .then(response => response.json())
                .then(data => {
                    const authorData = data.find(d => d.nombre_completo === authorName);
                    if (authorData) {
                        displayAuthorDetails(authorData);
                    } else {
                        alert("No data found for selected author.");
                    }
                })
                .catch(error => {
                    console.error("Error fetching author titles:", error);
                });
        }
        ...
        function displayAuthorDetails(authorData) {
            const modal = document.getElementById("category-modal");
            const modalTitles = document.getElementById("modal-titles");
            ...
            // Header content
            document.getElementById("modal-category-title").innerHTML = `Autor: <em>${authorData.nombre_completo}</em>`;
            document.getElementById("modal-frequency").textContent = authorData.Total_titles;
        
            // Populate list
            const titles = (authorData.titles || "").split("|| ").filter(Boolean);
