...
app.get('/api/sunburst-data', (req, res) => {
  // First get obras data
  const obrasSql = `
    SELECT 
      'Obra' AS tipo,
      META.género AS genero,
      THIRD.nombre_completo AS autor,
      META.Etapa_Dirección AS etapa,
      CAT.Categoría AS categoria,
      CAT.Forma AS forma,
      COUNT(DISTINCT META.ID_título) AS frecuencia
    FROM CATEGORIAS_TO_SERVER_3_Javier AS CAT
    JOIN Títulos_Metadata_Javier AS META ON CAT.ID_título = META.ID_título
    JOIN THIRD_MATCH_Javier AS THIRD ON META.ID_título = THIRD.ID_título
    WHERE META.Tipo_título = 'Obra'
    GROUP BY META.género, THIRD.nombre_completo, META.Etapa_Dirección, CAT.Categoría, CAT.Forma
  `;

  // Then get complementos data
  const complementosSql = `
    SELECT 
      'Complemento' AS tipo,
      'N/A' AS genero,
      'N/A' AS autor,
    ...
    FROM CATEGORIAS_TO_SERVER_3_Javier AS CAT
    JOIN Títulos_Metadata_Javier AS META ON CAT.ID_título = META.ID_título
    WHERE META.Tipo_título = 'Complemento'
    GROUP BY META.Etapa_Dirección, CAT.Categoría, CAT.Forma
  `;

  db2.serialize(() => {
    db2.all(obrasSql, [], (err, obrasRows) => {
      if (err) {
        console.error("SQL Error (obras):", err.message);
        return res.status(500).json({ error: err.message });
      }

      db2.all(complementosSql, [], (err, complementosRows) => {
        if (err) {
          console.error("SQL Error (complementos):", err.message);
          return res.status(500).json({ error: err.message });
        }

        const allRows = [...obrasRows, ...complementosRows];
        
        // Process the data into hierarchical format (same as before)
        const root = {
          name: "La Caprichosa",
          children: []
        };
		...
		const findOrCreateNode = (parent, name, level) => {
		let node = parent.children.find(child => child.name === name);
		if (!node) {
			node = { name, level };
			if (level < 6) { // Only add children array up to level 6
			node.children = [];
			} else {
				// For the deepest level, add frequency and titles
				const row = rows.find(r => 
					r.tipo === parent.parent?.parent?.parent?.parent?.name &&
					r.genero === parent.parent?.parent?.parent?.name &&
					r.autor === parent.parent?.parent?.name &&
					r.etapa === parent.parent?.name &&
					r.categoria === parent.name &&
					r.forma === name
				);
				if (row) {
					node.value = row.frecuencia;
					node.titulos = row.titulos.split('||').filter(t => t);
				}
				}
		...
		// Build the hierarchy
		rows.forEach(row => {
		// Level 1: Tipo (Obra/Complemento)
		const tipoNode = findOrCreateNode(root, row.tipo, 1);
		
		if (row.tipo === 'Obra') {
			// Level 2: Género (for Obras)
			const generoNode = findOrCreateNode(tipoNode, row.genero || 'ND', 2);
			
			// Level 3: Autor
			const autorNode = findOrCreateNode(generoNode, row.autor || 'Desconocido', 3);
			
			// Level 4: Etapa
			const etapaNode = findOrCreateNode(autorNode, row.etapa || 'Todas', 4);
			
			// Level 5: Categoría
			const categoriaNode = findOrCreateNode(etapaNode, row.categoria || 'Sin categoría', 5);
			
			// Level 6: Forma
			findOrCreateNode(categoriaNode, row.forma || 'Sin forma', 6);
		} else {
			// For Complementos, skip género and autor levels
			// Level 2: Etapa (directly under Complemento)
			const etapaNode = findOrCreateNode(tipoNode, row.etapa || 'Todas', 2);
			
			// Level 3: Categoría
			const categoriaNode = findOrCreateNode(etapaNode, row.categoria || 'Sin categoría', 3);
			
			// Level 4: Forma
			findOrCreateNode(categoriaNode, row.forma || 'Sin forma', 4);
		}
		});

		// Calculate values for parent nodes by summing children
		const calculateValues = (node) => {
		if (node.children) {
			node.value = 0;
			node.children.forEach(child => {
			calculateValues(child);
			node.value += child.value || 0;
			});
		}
		};
