...
def formatting_subtitles(text):
    ...
    return new_text
#!new function to handle century as category (NER doesn't recognize it)
...
#for element in processed_ent_es[:20]:
 # print(element, "\n")
...
#####---WITHOUT PROCESSING TITLES' PART CORRESPONDING TO NER---#####
...
for i_element, element in enumerate(pro_lem):
  for i_titulo, titulo in enumerate(element):
    pro_lem_cleaned[i_element][i_titulo] = [tupla for tupla in titulo
    if isinstance(tupla, tuple) and tupla[2] in ('NOUN', 'PROPN', 'ADJ', 'VERB', 'ADV', 'INTJ')]
...
'''for element in pro_lem_cleaned[:20]:
  print(element, "\n")
  print("============================")
  print("============================")'''

###--!!! ISSUE/QUESTION: ###
###-- DO WE WANNA KEEP NER CATEGORIES MERGED WITH LEM, POS WITHIN EACH TITLE?
###-- OR TWO SEPARATED LISTS TO APPLY (OR INTERATE ON) TO DB (LEM,POS PER TITLE | NER PER TITLE)????

#####---1 POSSIBLE SOLUTION: MERGING NER WITH LEM, POS PER TITLE: ---#####
#####---TO DO SO, WE MODIFY PROCESSING_2 FUNCTION---#####
def processing_2(text, nlp_model):
  doc = nlp_model(text)
  if text == "NULL":
    return [("NULL")]
  ...
  if "[(" in text:
    special_categories = re.findall(r'\[\(.*\)\]', text)
    parsing_text = re.split(r'\[\(.*\)\]', text)
    exception = []  #WE USE ZIP() TO HANDLE AN CORRELATE
    for part, special_cat in zip(parsing_text, special_categories + [None]):
      if special_cat:
        exception.append(special_cat)
      else:
        doc = nlp_model(part)
        exception.extend([(word.text, word.lemma, word.upos) for sent in doc.sentences for word in sent.words])
    return exception
...
#cleaning from irrelevant POS, as before:
pro_lem_cleaned_2 = copy.deepcopy(pro_lem)
for i_element, element in enumerate(pro_lem):
  for i_titulo, titulo in enumerate(element):
    pro_lem_cleaned_2[i_element][i_titulo] = [tupla for tupla in titulo if isinstance(tupla, tuple) and tupla[2] in ('NOUN', 'PROPN', 'ADJ', 'VERB', 'ADV', 'INTJ') or isinstance(tupla, str)]

for element in pro_lem_cleaned_2[:30]:
  print(element, "\n")

###--!!! SOMETHING WRONG WITH THIS NEW FUNCTION PROCESSING_2()
###-- IN SOME TITLES NOT CATCHING TOKS AND LEMMAS BEFORE THE FIRST NER
###--!!! CHECK THIS IN ORDER TO PROCEED RIGHTLY. ASK NADÈGE 
###--(28.03)